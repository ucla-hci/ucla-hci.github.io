...
UCLAHCI.checkResponsiveness = function () {
	if ($(window).width() < 640 || window.mobilecheck() == true) {
		...
		// $('div.topnav').css('line-height', UCLAHCI.config.LOGOHEIGHT);
	} else {
		// $('div.topnav').css('line-height', UCLAHCI.config.LOGOHEIGHT);
	}
}
...
$(document).ready(function () {
    YAML.load('config.yml', function (result) {
        ...
        // $('div.topnav').css('height', UCLAHCI.config.LOGOHEIGHT);
        // $('img.logo').attr('src', UCLAHCI.config.LOGOURL)
        // $('img.logo').css('height', UCLAHCI.config.LOGOHEIGHT);
